...
export async function POST(req: Request) {
    ...
    try {
        const body: ThruFloorsRequest = await req.json();
        ...
        const response = await sendRconCommand(host, port, `cv thrufloors ${enabled ? 1 : 0}`);
        return NextResponse.json({ success: true, response });
    } catch (error) {
        console.error('RCON thrufloors error:', error);
