...
export async function POST(req: Request) {
    ...
    try {
        const body: KickBotsRequest = await req.json();
        ...
        const response = await sendRconCommand(host, port, 'kickbots');
        return NextResponse.json({ success: true, response });
    } catch (error) {
        console.error('RCON kickbots error:', error);
