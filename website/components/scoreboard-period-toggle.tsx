...
export function ScoreboardPeriodToggle({period, onChange, mode}: ScoreboardPeriodToggleProps) {
    return (
        <div
            className="inline-flex flex-wrap items-center gap-1 rounded-lg border border-border/60 bg-background/70 p-1">
            {Object.keys(ScoreboardPeriod).filter(v => !(mode === "kd" && v === "DAILY")).reverse().map((value) => (
                <Button
                    key={value}
                    type="button"
                    size="sm"
